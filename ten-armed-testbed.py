...
np.random.seed(0) 
step_size = 1
def stationary_ten_armed_testbed(eps, q_estimates , num_steps = 1000, initial_reward = 0):
...
def nonstationary_ten_armed_testbed(eps, q_estimates , num_steps = 1000, initial_reward = 0):
	optimal_move = np.argmax(q_estimates) 
	moves_count = np.zeros(10) 
	rewards_till_now = [] 
	reward = 0 
...
def UCBstationary_ten_armed_testbed(eps, q_estimates , num_steps = 1000,c = 2):
	optimal_move = np.argmax(q_estimates) 
	moves_count = np.zeros(10)
	rewards_till_now = [] 
	reward = 0 
...
def UCBnonstationary_ten_armed_testbed(eps, q_estimates , num_steps = 1000,c = 2):
	optimal_move = np.argmax(q_estimates) 
	moves_count = np.zeros(10)
	rewards_till_now = [] 
	reward = 0 
...
if __name__ =='__main__':
	...
	eps = [0 , 0.1, 0.01]
	ckpts = [i for i in range(1,1001) if i % step_size == 0] 
	Q_estimates = sp.random.standard_normal(size=10) 
	# for e in eps:
	# 	avg_trail = np.zeros(1000 // step_size) 
	# 	avg_optimality = np.zeros(1000 // step_size)
	# 	for _ in range(num_trials):
	# 		print('{}/2000 trials done'.format(_), end = '\r')
	# 		rewards, optimality = nonstationary_ten_armed_testbed(e, Q_estimates[:])
	...
	avg_optimistic = np.zeros(1000 // step_size) 
	avg_notoptimistic = np.zeros(1000 // step_size)  
	for _ in range(num_trials):
		__ , optmistic =  stationary_ten_armed_testbed(0, Q_estimates[:], initial_reward = 5)
		__ , not_optmistic = stationary_ten_armed_testbed(0.1, Q_estimates) 
		__ , ucb = UCBstationary_ten_armed_testbed(0,Q_estimates)
	...
	plt.plot(ckpts,ucb,label = 'UCB, c = 2')
	plt.legend()
	plt.show()
